...
import com.example.ramadhan.jadwalimsak.GetAddressIntentService
import com.example.ramadhan.jadwalimsak.MainAdapter.onSelectData
...
class MainActivity_Jadwal_Imsak : AppCompatActivity(), onSelectData {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        setContentView(R.layout.layout_jadwal_imsak)
    ...
    private val textJSON: Unit
        get() {
            val progressDialog = ProgressDialog(this@MainActivity_Jadwal_Imsak)
                    ...
                    .getAsJSONObject(object : JSONObjectRequestListener {
                        override fun onResponse(response: JSONObject) {
                            try {
                                ...
                                e.printStackTrace()
                                Toast.makeText(this@MainActivity_Jadwal_Imsak, "Gagal menampilkan data!", Toast.LENGTH_SHORT).show()
                            }
                        }

                        override fun onError(anError: ANError) {
                            progressDialog.dismiss()
                            Toast.makeText(this@MainActivity_Jadwal_Imsak, "Tidak ada jaringan internet!", Toast.LENGTH_SHORT).show()
                        }
                    })
        }
    ...
    private fun showNews() {
        val mainAdapter = MainAdapter(this@MainActivity_Jadwal_Imsak, modelMainList, this)
        rvList!!.adapter = mainAdapter
    }

    override fun onSelected(modelMain: ModelMain) {
        val intent = Intent(this@MainActivity_Jadwal_Imsak, DetailActivity::class.java)
    ...
    private val address: Unit
        get() {
            if (!Geocoder.isPresent()) {
                Toast.makeText(this@MainActivity_Jadwal_Imsak, "Can't find current address, ", Toast.LENGTH_SHORT).show()
                return
            }
    ...
    private inner class LocationAddressResultReceiver internal constructor(handler: Handler?) : ResultReceiver(handler) {
        override fun onReceiveResult(resultCode: Int, resultData: Bundle) {
            ...
            if (resultCode == 1) {
                Toast.makeText(this@MainActivity_Jadwal_Imsak,
                        "Address not found, ",
