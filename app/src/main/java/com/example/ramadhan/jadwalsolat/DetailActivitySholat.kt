...
class DetailActivitySholat : AppCompatActivity() {

    var modelMain: ModelMainSholat? = null
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        setContentView(R.layout.activity_detailsholat)
        ...
        //model to show data
        modelMain = intent.getSerializableExtra("paramDtl") as ModelMainSholat
        if (modelMain != null) {
    ...
    private val address: Unit
        get() {
            if (!Geocoder.isPresent()) {
                Toast.makeText(this@DetailActivitySholat, "Can't find current address!", Toast.LENGTH_SHORT).show()
                return
            }
    ...
    private inner class LocationAddressResultReceiver internal constructor(handler: Handler?) : ResultReceiver(handler) {
        override fun onReceiveResult(resultCode: Int, resultData: Bundle) {
            ...
            if (resultCode == 1) {
                Toast.makeText(this@DetailActivitySholat, "Address not found!", Toast.LENGTH_SHORT).show()
            }
